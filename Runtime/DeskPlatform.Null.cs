#if !(UNITY_EDITOR_WIN || (UNITY_STANDALONE_WIN && !UNITY_EDITOR) || UNITY_EDITOR_OSX || (UNITY_STANDALONE_OSX && !UNITY_EDITOR))
...
namespace LifeLogs.WindowUtil {
    ...
    /// <summary>
    /// 데스크탑 창 제어를 지원하지 않는 플랫폼(Android, iOS, WebGL, Linux 등)용 no-op 구현.
    /// 모바일 빌드에서 컴파일 에러가 나지 않도록 자리를 채우는 것이 목적입니다.
    /// </summary>
    internal static class DeskPlatform {

        private const string UNSUPPORTED_MESSAGE =
            "[DeskPlatform.Null] Desktop window control is not available on this platform.";
        ...
        /// <summary>데스크탑 창 개념이 없는 플랫폼이므로 제공하는 기능이 없습니다.</summary>
        ...
        internal static bool TryEnumerateMonitors(List<DeskMonitorInfo> buffer, out Exception error) {
            WarnOnce();
            error = new PlatformNotSupportedException(UNSUPPORTED_MESSAGE);
            return false;
        }
